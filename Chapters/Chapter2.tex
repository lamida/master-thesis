...
\ft{DON'T USE FUTURE TENSE. EVERYTHING IN PRESENT TENSE. SIMPLE PAST AT MOST. 
check it everywhere.}
...
\ft{don't call them software attack, these are ``Control flow attacks''}
...
\ft{do you use ``in the past'', and then ``will'' (?) just use past tense :D}
...
Prevention~\ft{cite}. A return-oriented program chains together short 
instruction sequences already 
present in a program’s address space, each of which ends in a “return”
\ft{use \texttt{ret} (with the texttt tag) instead of ``return''. Moreover, the 
quotes in latex are made with `` and ''. Not ".}
 instruction. ROP can't be mititgated by \( W \bigoplus R \)~\ft{cite}.
...
and optimizations on tools called opt.\ft{write \texttt{opt} for tool name.}
...
\ft{for all functions, use \texttt{parseIR()}, also for file name 
\texttt{IIReader.h} } parsing IR using `parseIR` or 
`parseIRFile` from 
